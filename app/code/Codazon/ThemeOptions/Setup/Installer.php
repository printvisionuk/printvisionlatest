...
class Installer implements Setup\SampleData\InstallerInterface
{
    ...
    public function __construct(
        ...
        \Codazon\ThemeOptions\Framework\App\Config\Initial\Reader $themeConfig,
    ...
    protected function assignTheme()
    {
        ...
        foreach ($themes as $theme) {
            if(substr($theme->getCode(), 0, strlen($query)) === $query)
            {
                ...
                $data = $this->themeConfig->read($theme->getThemeId());
                $cmsHomePage = $data['data']['default']['cms_home_page'];
                //$cmsHomePage = $this->themeConfig->getValue('cms_home_page', \Magento\Store\Model\ScopeInterface::SCOPE_STORE);s

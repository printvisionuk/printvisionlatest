...
class Option extends \Magento\CatalogImportExport\Model\Import\Product\Option
{
    ...
    protected function _importData()
    {
        ...
        while ($bunch) {
            ...
            if (empty($products)) {
                $lastProduct = end($oldProducts);
                $products[key($oldProducts)] = $lastProduct;
            }
            if (empty($options)) {
                $lastOption = end($oldOptions);
                $options[key($oldOptions)] = $lastOption;
            }
            if (empty($titles)) {
                $lastTitle = end($oldTitles);
                $titles[key($oldTitles)] = $lastTitle;
            }
    ...
    private function savePreparedCustomOptions(
        ...
        if ($this->getBehavior() == ImportModel::BEHAVIOR_APPEND) {
            $this->optionIdsMap = array_flip($this->optionIdsMap);
            $this->valueIdsMap  = array_flip($this->valueIdsMap);
    ...
    /**
     * Rewrite function for saving more then 100 values
     *
     * @param array $typeValues
    ...
    protected function _saveSpecificTypeValues(array $typeValues)
    {
        ...
        foreach ($typeValues as $optionId => $optionInfo) {
            foreach ($optionInfo as $row) {
                if (array_search($row['option_type_id'], $optionTypeIds) === false) {
                    $row['option_id'] = $optionId;
                    $optionTypeIds[]  = $row['option_type_id'];
                    $typeValueRows[]  = $row;
                }
        ...
        if ($typeValueRows) {
            $this->deleteSpecificTypeIds($optionTypeIds);
    ...
    /**
     * Delete custom option type ids
     *
     * @param array $optionTypeIds
     * @return $this
     */
    protected function deleteSpecificTypeIds(array $optionTypeIds)
    {
        $this->_connection->delete(
            $this->_tables['catalog_product_option_type_value'],
            $this->_connection->quoteInto('option_type_id IN (?)', $optionTypeIds)
        );
        ...
        foreach ($typeValues as $optionId => &$optionTypes) {
            foreach ($optionTypes as &$optionType) {
                ...
                foreach ($typeTitles[$optionTypeId] as $storeId => $optionTypeTitle) {
                    ...
                    if ($existingTypeId) {
                        if (isset($this->valueIdsMap[$optionType['option_type_id']])) {
                            $this->convertedValueIdsMap[$this->valueIdsMap[$optionType['option_type_id']]] = (int)$existingTypeId;
                        }
    ...
    protected function _compareOptionsWithExisting(array &$options, array &$titles, array &$prices, array &$typeValues)
    {
        foreach ($options as &$optionData) {
            ...
            if ($optionId = $this->_findExistingOptionId($optionData, $titles[$newOptionId])) {
                if (isset($this->optionIdsMap[$newOptionId])) {
                    $this->convertedOptionIdsMap[$this->optionIdsMap[$newOptionId]] = (int)$optionId;
                }
